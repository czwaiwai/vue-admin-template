  ...
  <el-dialog :visible.sync="visible" :before-close="modalClose" :title="'门店号：'+formObj.short+ ' 门店名称：'+formObj.title" width="80%" @open="modalOpen" >
    <div>
      <el-table
        ...
        <el-table-column class-name="status-col" label="收益比例" align="center">
          <template slot-scope="scope">
            <p class="line-clamp2"> {{ scope.row.num }}%</p>
          </template>
        </el-table-column>
        ...
        <el-table-column class-name="status-col" label="单件利润（平台）" align="center">
          <template slot-scope="scope">
            <p class="line-clamp2"> {{ scope.row.num }}%</p>
          </template>
        </el-table-column>
        <el-table-column class-name="status-col" label="单件收益（门店）" align="center">
          <template slot-scope="scope">
            <p class="line-clamp2"> {{ scope.row.num }}%</p>
          </template>
        </el-table-column>
        <el-table-column class-name="status-col" label="销量" align="center">
          <template slot-scope="scope">
            <p class="line-clamp2"> {{ scope.row.num }}</p>
          </template>
        </el-table-column>
        <el-table-column class-name="status-col" label="已兑换" align="center">
          <template slot-scope="scope">
            <p class="line-clamp2"> {{ scope.row.num }}</p>
          </template>
        </el-table-column>
        <el-table-column class-name="status-col" label="剩余库存" align="center">
          <template slot-scope="scope">
            <p class="line-clamp2"> {{ scope.row.num }}</p>
          </template>
        </el-table-column>
